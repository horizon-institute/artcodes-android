...
class TWMarkerSurfaceView extends TWSurfaceViewBase
{
	 ...
	 * 
	 * @param imgMat
	 *            Source image from which to compute image segment.
	...
	private Rect calculateImageSegmentArea(Mat imgMat)
	{
		...
		// Size of width and height varies of input image can vary. Make the
		// image segment width and height equal in order to make
	...
	// {
	// // find location of image segment to be replaced in the destination
	// // image.
	// Rect rect = calculateImageSegmentArea(destImageMat);
	// // Mat destSubmat = destImageMat.submat(rect.y, rect.y + rect.height,
	// // rect.x, rect.x + rect.width);
	// // Mat destSubmat = destImageMat.submat(rect.y, rect.y + rect.height /
	// // 2, rect.x, rect.x + rect.width / 2);
	// Mat destSubmat = destImageMat.submat(rect.y, rect.y + rect.height,
	// rect.x, rect.x + rect.width);
	// // copy image.
	...
	private ArrayList<Double> applyThresholdOnImage(Mat srcImgMat, Mat outputImgMat)
	{
		...
		for (int tileRowCount = 0; tileRowCount < numberOfTiles; tileRowCount++)
		{
			...
			for (int tileColCount = 0; tileColCount < numberOfTiles; tileColCount++)
			{
				...
				// 2);
				Imgproc.threshold(tileMat, tileThreshold, 127, 255, Imgproc.THRESH_BINARY | Imgproc.THRESH_OTSU);
	...
	// Core.putText(imgMat, code, codeLocation, Core.FONT_HERSHEY_COMPLEX, 1,
	// codesColor,3);
	//
	...
	private void displayMarkersDebug(Mat imgMat, Scalar contourColor, Scalar codesColor)
	{
		...
		if (markerFound)
		{
			...
			String code = codeArrayToString(marker.getCode());
			// Point codeLocation = new Point(imgMat.cols() / 4, imgMat.rows() /
			// 8);
			Point codeLocation = new Point(imgMat.cols(), imgMat.rows());
			Core.putText(mRgba, code, codeLocation, Core.FONT_HERSHEY_COMPLEX, 1, codesColor, 3);

			// 4= Thickness, 8 =linetype

			Imgproc.drawContours(mRgba, components, marker.getComponentIndex(), contourColor, 4, 8, hierarchy, 2, new Point(0, 0));
		...
		try
		{
			...
			while (Thread.currentThread() == mThread)
			{
				...
				if (bmp != null && mCamera != null)
				{
					...
					if (canvas != null)
					{
						// RNM rotate to receive landscape camera in portrait
						// display
						canvas.rotate(90);
