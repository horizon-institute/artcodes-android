...
public class WhiteBalanceComponent implements ImageProcessingComponent
{
    MatOfInt[] channels = null;
    Mat[] histograms = null;
    Mat emptyMatMask = null;
    MatOfInt size = null;
    MatOfFloat range = null;
    Mat lutBuffer = null;
    ...
    {
        Mat image = bufferManager.getMostRecentDataInBgr();
        ...
        if (lutBuffer==null)
        {
            lutBuffer = new Mat(1, 256, CvType.CV_8UC3);
        }
        Mat lut = lutBuffer;
        final int lutSize = lut.cols() * lut.rows() * lut.channels();
        int lutIndex = -1;
        byte[] lutBuffer = bufferManager.getByteArray(lutSize);
        for (int i=0; i<256; ++i)
        {
            for (int channel=0; channel<image.channels(); ++channel)
            {
                lutBuffer[++lutIndex] = (byte) Math.min(Math.max(a[channel] * ((i) - b[channel]), 0), 255);
            }
        }
        lut.put(0, 0, lutBuffer);
        bufferManager.returnByteArray(lutBuffer);
